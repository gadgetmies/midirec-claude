...
Define the session organization model where channels group a piano-roll track and zero-or-more CC lanes. Provide the React orchestrator that renders one channel group per visible channel inside the timeline, and the global solo/mute composition rules that span the channel stack.
