...
- **`demo=instrument`**, **`demo=marquee`**, and **`demo=note`** each imply the **instrument fixture** (Lead roll notes) per the `channels` capability; **only `demo=marquee` additionally drives piano-roll selection from the URL**:
  ...
  - **When `demo=marquee` is absent** (including **`/?demo=note`** without marquee): **`marquee` SHALL be `null`**. **`selectedChannelId`** and **`selectedIdx`** SHALL follow **`interactiveRollSel`** / **`selectRollNote`** unless another capability overrides them; on a fresh load with no gestures, **`selectedIdx`** is empty, **`selectedChannelId` MAY be **`null`**, and **`resolvedSelection` SHALL be `null`**.

Expose **`selectRollNote`** alongside existing stage API so instrument-roll **`onNoteSelect`** can mutate **`selectedIdx` / `selectedChannelId`** without bypassing derivation invariants documented above.
...
#### Scenario: demo=note loads instrument without URL-driven selection

- **WHEN** the app is loaded at `/?demo=note`
- **THEN** the rendered DOM SHALL NOT contain any `.mr-marquee` element
- **AND** `useStage().resolvedSelection` SHALL equal **`null`** before any **`selectRollNote`** / note-click gesture
- **AND** no `.mr-note` element SHALL carry `data-selected="true"` unless the user interacts
...
Orchestration code outside `PianoRoll` **SHALL** wire this callback such that activating a note on a roll whose `channelId === C` sets **`selectedChannelId = C`** and **`selectedIdx` to `[i]`**. Track-header **`data-timeline-selected`** state SHALL reflect header clicks independently (note activation **SHALL NOT** imply choosing the channel/track header timeline focus).
