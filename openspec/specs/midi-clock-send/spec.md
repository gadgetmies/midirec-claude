# midi-clock-send Specification

## Purpose

Define the outbound MIDI Clock Send pipeline: a `MidiClockSendProvider` that emits 24-PPQ System Real-Time Clock (`0xF8`) pulses to user-selected `MIDIOutput` ports, transport-driven Start/Continue/Stop (`0xFA`/`0xFB`/`0xFC`) messages, a Sync action that emits a Stop + Song Position Pointer + Continue/Start realignment bundle, and a Grid Alignment trigger that emits configurable Note or CC messages on bar or phrase boundaries — without interfering with concurrent non-clock MIDI traffic on the same ports.
