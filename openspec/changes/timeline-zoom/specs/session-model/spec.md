...
- **View geometry** — the field `pxPerBeat: number` from `StageState` (per the `timeline-zoom` capability). This is a presentation-state datum, not a musical datum, but it SHALL persist so that re-opening a session restores the user's chosen horizontal zoom.
...
- **AND** the payload SHALL include a numeric `pxPerBeat` field reflecting `useStage().pxPerBeat` at serialisation time
...
#### Scenario: pxPerBeat round-trips through persistence

- **GIVEN** an editor state where `useStage().pxPerBeat === 250`
- **WHEN** the state is serialised and then deserialised through the storage layer
- **THEN** `useStage().pxPerBeat` SHALL equal `250` after hydration
